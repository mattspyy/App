      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {record.paymentMethod && <Tag tone="blue">{record.paymentMethod}</Tag>}
            {record.status === "needs_review" && <Tag tone="amber">Needs review</Tag>}
            {record.status === "draft" && <Tag tone="rose">Draft</Tag>}
          </div>
          <div className="font-medium text-zinc-900 truncate">{record.merchant || "—"}</div>
