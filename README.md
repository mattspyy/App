...
- **2026-05-22 — UI redesign · Phase 2** *(branch: `ui-redesign`)*. Redesigned the Add flow and History on the FXT tokens. `/scan` now splits the four entry methods into "From an image" (Scan, Upload) and "By text" (Smart Add, Manual Add) with a calm in-progress checklist for AI extraction. `/scan/smart-add` switched to a Card-wrapped textarea with character counter, clickable example prompts, and a quiet "Parsing…" state. `/scan/confirm` was restructured into three primary Cards (Essentials, Category & payment, Split), a collapsible "More details" section for country/spread/payer/notes, a separate Receipt-items Card, and a sticky `BottomActionBar` — preserving all original logic (Personal-group fallback, Smart-Add hint resolution, category-rule auto-apply, duplicate detection, offline-fallback save). `/history` now uses `PageHeader` + `SelectChip` filters in a tone-soft Card with a filtered-total chip; day headers use serif weekday + mono totals. No route, API, data-model, or feature changes. Manual Add stays Manual Add.
- **2026-05-22 — UI redesign · Phase 3** *(branch: `ui-redesign`)*. Redesigned the remaining seven pages on the FXT tokens. **Group Detail** now uses a `PageHeader` with eyebrow indicating Personal / Private / Public, member rows with avatars + invite codes, an inline invite form for private-group admins, an optional "Trips in this group" grid (filtered from `/api/trips` by `familyId`), and keeps the existing `MonthlyBudgetCard`, `DashboardCards`, `CategoryPieChart`, `UserBarChart`, `SpendingLineChart`, and `RecordsTable`. **Trips List** groups trips into Active / Upcoming / Past / No-dates buckets with a colored day badge per card. **New Trip** uses three Cards (Basics, Money, Notes) with destination-currency auto-detect surfaced as a sage badge. **Trip Detail** keeps the gradient banner, adds a budget-pace strip, four `StatCard`s (Today / Trip total / Budget % / Day status) with progress, action buttons, Today / Recent / Breakdown / Daily-trend sections, danger zone. **Settlement** is restructured into Suggested payments (one card per pair), Per-person balance, and Recorded payments (now resolves user IDs to display names via the balances map). A client-side **Copy summary** button uses `navigator.clipboard.writeText` — no backend changes. **Trip Report** is split into Summary stat cards, Charts (line + payment method), Breakdown tables (by category / by person), Rankings, Settlement summary, and a full records table; date range filter + CSV export preserved. **Settings** now has Account / Display currency / Session sections, an unsaved badge, and a conditional Admin section that only renders for usernames in `NEXT_PUBLIC_ADMIN_USERNAMES`. No route, API, data-model, or feature changes. Phase 1–3 complete; all 16 page-level routes now use the FXT design system.
- **2026-05-22 — Language settings and localized UI copy** *(branch: `ui-redesign`)*. Added an app language system supporting 繁體中文 (zh-HK, default), English (en), and 简体中文 (zh-CN). New `src/lib/i18n.ts` provides a `useLanguage()` hook, `setLanguage()` persistor (localStorage `fxt.language`), `translate()` helper, and three dictionaries; default falls back to Traditional Chinese. Added `LanguageSync` client component to keep `document.documentElement.lang` in sync. Replaced the large editorial login/register headline with simpler app-focused hero copy: brand "FXT" + tagline (e.g. 「輕鬆記低每一筆開支」) + subtext. Login, Register, Nav (top bar + bottom nav), Groups List, Settings (with a new Language section), Scan/Add menu, and Smart Add now use the translation system. Settings shows three radio options with a sage `Saved ✓` badge on change. Internal routes, APIs, and data models remain unchanged. Not every deep UI string is translated yet (Confirm, History, Trip Detail, Settlement, Report still ship existing English copy) — translation is incremental and additional surfaces can adopt `t()` without code restructuring.
- **2026-05-22 — Login/Register PIN validation fix** *(branch: `ui-redesign`)*. Fixed a regression where the PIN input's HTML `pattern` attribute was emitted with a double-escaped backslash (`pattern="\\d{4}"`), causing the browser to reject every valid 4-digit PIN with "請符合要求的格式。" Removed the broken HTML pattern from `/login` and `/register`, added React-level validation before submit (`/^\d{4}$/`), and added new translations `login.errorInvalidPin` and `register.errorInvalidPin` (zh-HK / en / zh-CN) so the error surfaces as an inline `Alert` instead of a native browser popup. The PIN input still sanitizes input to digits and clamps to 4 characters via `replace(/\D/g, "").slice(0, 4)`. No auth-backend, API, or route changes.
- **2026-05-22 — Full localization audit and migration** *(branch: `ui-redesign`)*. Expanded the i18n system to cover every primary user-facing surface across the app. Added ~250 new translation keys grouped by area (`common.*`, `actions.*`, `states.*`, `errors.*`, `groups.detail.*`, `groups.new.*`, `trips.*`, `tripsNew.*`, `tripDetail.*`, `settlement.*`, `report.*`, `history.*`, `confirm.*`, `budget.*`, `expenseCard.*`, `records.*`, `ranking.*`, `dashboard.*`, `charts.*`, `pendingSync.*`, `upload.*`) with full translations in 繁體中文 (zh-HK, default), English (en), and 简体中文 (zh-CN). Added label-mapper helpers `categoryLabel`, `paymentMethodLabel`, `splitTypeLabel`, `sourceTypeLabel` that translate the display label while keeping the stored enum value (e.g. `"Food"`, `"Cash"`, `"no_split"`, `"receipt"`) unchanged. Migrated the following pages to use the translation system: `/`, `/onboarding`, `/family`, `/parties/new`, `/parties/[partyId]`, `/history`, `/trips`, `/trips/new`, `/trips/[tripId]`, `/trips/[tripId]/settlement`, `/trips/[tripId]/report`, `/scan/confirm`. Migrated the following shared components: `Nav`, `ExpenseCard`, `RecordsTable`, `RankingLists`, `DashboardCards`, `CategoryPieChart`, `PaymentMethodChart`, `SpendingLineChart`, `UserBarChart`, `MonthlyBudgetCard`, `PendingSyncNotice`, `UploadBox`. Dynamic values (counts, currencies, names, dates, percentages) flow through the existing `{var}` interpolation. PIN browser-validation popup remains app-level (was fixed in the prior changelog entry) and PIN inputs continue to sanitize to 4 digits with localized inline errors. Internal route paths, stored database values, and API field names are unchanged.

Notes:
- Confirm/Edit page chrome (section headers, field labels, error messages, alerts) is translated. Some uncommon micro-helpers (split preview details, daily spread daily-allocated-amount strings, etc.) may still surface English text and can be migrated incrementally.
- `/admin` page kept in English (operator-only surface, password-gated).
- CSV export column headers remain English so external spreadsheets stay compatible.
- Browser-native UI like `<input type="date">` calendar uses the OS locale, not the FXT language setting.
